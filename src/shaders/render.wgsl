...
fn vs_main(
    ...
    let tangent = model.tangent.xyz;
    let bitangent = model.tangent.w * cross(model.normal, tangent);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Original Phong method:
    // let reflect_dir =  reflect(-light_dir, in.world_normal);
    // let specular_strength = pow(max(dot(view_dir, reflect_dir), 0.0), 32.0);

    // Modified Blinn method:
    let half_dir = normalize(view_dir + light_dir);
    let specular_strength = pow(max(dot(tangent_normal, half_dir), 0.0), 250.0);

    let specular_color = 0.15 * specular_strength * light.color;
